...
- ***Always* Start your execution with:**
  ...
  3. Write clear, concise, and well-documented code.
    - **"Well-documented" means specifically:**
      ...
      - Check existing XML docs to be up-to-date *and* usage of `<see cref="..." />` Tags with Type/Symbol referenced instead of writing the Identifyer as pure string.
...
- If a Interface defines a Task or ValueTask returning Method, you **MUST** implement it as async Method in the Implementing Class, unless explicitly requested otherwise!
- Do **NOT** use `Task.FromResult(...)` or `ValueTask.FromResult(...)` in Implementations of Interface Methods! The interface but it can not tell you if the Implementation will be async or not.
...
- Avoid expanding public API surface unnecessarily. This means:
  ...
  - This also applyes to helper methods! Keep them private unless needed externally
...
- In case you got a typed argument, which *can not* simply provided to following Methods as argument or its provided Values need to be used as part of a MVUX State `.ForEach(...)`-Callback-Method
  - Apply this following Instructions:
    ...
    - **Example from this Codebase:**
      ...
      2. This Object now gots provided to the `IOAuthNavigationService.NavigateAuthenticationAsync(` as Method-Parameter -> in this Service the object don't needs to get used future then transfering it. Here you **WILL NOT** store it or its Properties into backing fields.
      ...
      4.  private helper Classes needs this Object.
      - **Prefered Approach:** If the helper Method is *directly* called by your Method, simply provide it as argument for the called Method.  into separate fields when an existing context object (e.g.,  with it's Properties `State` / `CodeVerifier`) already carries them; 
...
- Prefer `async` suffix for asynchronous methods instead of `Task.FromResult`, except from this must be called by a constructor or Eventhandler Method.
